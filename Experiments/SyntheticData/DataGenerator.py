...
nameFile = 'DATA' + 'C' + str(classes) + 'F' + str(Features) + 's' + str(seed) + 'S' + str(samples) + 'P' + str(
    people) + 'sh' + str(shots) + 'cR' + str(covRandom) + 'mR' + str(meanRandom) + 't' + str(times) + 'mix' + str(mix)


def DataGenerator(classes=2, Features=2, seed=None, samples=100, people=5, Graph=True, covRandom=True, meanRandom=True,
                  nameFile='default.csv', save=True, mix=False):
    classCovFactor = 1
    vU = 1
    np.random.seed(seed)
    random.seed(seed)
        ...
        for cl in range(classes):
            ...
            if Graph:
                ...
                if person == people - 1 or (mix and (person == people / 2 - 1)):
                    ...
                    CF.confidence_ellipse(x1, x2, ax, edgecolor=color, label=label)
                else:
                    ax.scatter(x1, x2, s=0.5, color=color)
                    CF.confidence_ellipse(x1, x2, ax, edgecolor=color)
...
DataGenerator(classes, Features, seed, samples, people, Graph, covRandom, meanRandom, nameFile, save, mix)
