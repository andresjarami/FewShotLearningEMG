...
# startPerson=6
# endPerson=6
# nameFile='pruebaCoteNew.csv'



def preTrainedData(dataMatrix,classes,peoplePreTrain,allFeatures):
    preTrainedDataMatrix = pd.DataFrame(columns=['mean','cov','class','person'])
    typeData=0
    indx=0
    for cl in range(0,classes):

        for person in range(0,peoplePreTrain):

            preTrainedDataMatrix.at[indx,'cov']= np.cov(dataMatrix[np.where((dataMatrix[:,allFeatures]==typeData)*(dataMatrix[:,allFeatures+1]==person)*(dataMatrix[:,allFeatures+3]==cl)),0:allFeatures][0],rowvar=False)
            preTrainedDataMatrix.at[indx,'mean'] = np.mean(dataMatrix[np.where((dataMatrix[:,allFeatures]==typeData)*(dataMatrix[:,allFeatures+1]==person)*(dataMatrix[:,allFeatures+3]==cl)),0:allFeatures][0],axis=0)
...
def currentDistributionValues(trainFeatures,trainLabels,classes,allFeatures):
    ...
    for cla in range(0,classes):
        currentValues.at[cla,'cov']=np.cov(Matrix[np.where((Matrix[:,allFeatures]==cla)),0:allFeatures][0],rowvar=False)
        currentValues.at[cla,'mean']=np.mean(Matrix[np.where((Matrix[:,allFeatures]==cla)),0:allFeatures][0],axis=0)
        currentValues.at[cla,'class']=cla
...
def weightDenominatorLiu(currentMean,preTrainedDataMatrix):
    ...
    for i in range(len(preTrainedDataMatrix.index)):
        weightDenominatorV=weightDenominatorV+(1/distance.mahalanobis(currentMean,preTrainedDataMatrix['mean'].loc[i],np.linalg.inv(preTrainedDataMatrix['cov'].loc[i])))
...
def reTrainedMeanLiu(r,currentMean,preTrainedDataMatrix,weightDenominatorV,allFeatures):
    ...
    for i in range(len(preTrainedDataMatrix.index)):
        sumAllPreTrainedMean_Weighted=np.add(sumAllPreTrainedMean_Weighted,preTrainedDataMatrix['mean'].loc[i]*(1/distance.mahalanobis(currentMean,preTrainedDataMatrix['mean'].loc[i],np.linalg.inv(preTrainedDataMatrix['cov'].loc[i]))))
...
def reTrainedCovLiu(r,currentMean,currentCov,preTrainedDataMatrix,weightDenominatorV,allFeatures):
    ...
    for i in range(len(preTrainedDataMatrix.index)):
        sumAllPreTrainedCov_Weighted=np.add(sumAllPreTrainedCov_Weighted,preTrainedDataMatrix['cov'][i]*(1/distance.mahalanobis(currentMean,preTrainedDataMatrix['mean'][i],np.linalg.inv(preTrainedDataMatrix['cov'][i]))))
...
def reTrainigLiu(currentValues,preTrainedDataMatrix,classes,allFeatures):
    ...
    r=0.5
    for cla in range(0,classes):
        preTrainedMatrix_Class=pd.DataFrame(preTrainedDataMatrix[['cov','mean']].loc[(preTrainedDataMatrix['class'] == cla)])
        ...
        trainedModel.at[cla,'cov']=reTrainedCovLiu(r,currentMean,currentCov,preTrainedMatrix_Class,weightDenominatorV,allFeatures)
        trainedModel.at[cla,'mean']=reTrainedMeanLiu(r,currentMean,preTrainedMatrix_Class,weightDenominatorV,allFeatures)[0]
        trainedModel.at[cla,'class']=cla
...
def reTrainigProposed10BB(currentValues, preTrainedDataMatrix, classes,allFeatures):
    ...
    for cla in range(0, classes):
        preTrainedMatrix_Class = pd.DataFrame(
            preTrainedDataMatrix[['cov', 'mean']].loc[(preTrainedDataMatrix['class'] == cla)])
        preTrainedMatrix_Class = preTrainedMatrix_Class.reset_index(drop=True)
        currentCov = currentValues['cov'].loc[cla]
        currentMean = currentValues['mean'].loc[cla]
...
def LDA_Cov(trainedModel, classes):
    allFeatures=len(trainedModel['cov'].loc[0])
...
def SamplesFew_ShotTrainedModels(model, samples, classes):
    ...
    for cl in range(0, classes):
        ...
        yTrain.extend((cl) * np.ones(samples))
    return xTrain, yTrain




def evaluation(dataMatrix,segment,classes,peoplePreTrain,peopleTrain,CH,numberFeatures,featureSet,numberShots,combinationSet
    ...
    preTrainedDataMatrix=preTrainedData(dataMatrix,classes,peoplePreTrain,allFeatures)
    # Number of samples per shot
    samples=1000
    
    for person in range(peoplePreTrain+startPerson-1,peoplePreTrain+endPerson):
        carpet=2
        testFeatures=dataMatrix[np.where((dataMatrix[:,allFeatures]==typeData)*(dataMatrix[:,allFeatures+1]==person)
                     *(dataMatrix[:,allFeatures+2]==carpet)),0:allFeatures][0]
        testLabels=dataMatrix[np.where((dataMatrix[:,allFeatures]==typeData)*(dataMatrix[:,allFeatures+1]==person)
...
# # Cote Allard Data
resultsParameters = pd.DataFrame(columns=['pBest', 'n_neighborsBest', 'weightsBest','CBest','kernelBest','gammaBest'])
Index=0
# ## Feature Set 1 (MAV)
...
if featureSet==1:
    ...
    featureSet=1
    numberShots=4
    combinationSet=[1,2,3,4]
    # Getting Data /content/drive/My Drive/Jupyter/ExtractedData
    ...
    # Getting general data from PreTraining Data: typeData=0
    typeData=0
    generalDataFeatures=dataMatrix[np.where((dataMatrix[:,allFeatures]==typeData)),0:allFeatures][0]
    generalDataLabels=dataMatrix[np.where((dataMatrix[:,allFeatures]==typeData)),allFeatures+3][0]
    ...
    resultsParameters=pd.read_csv('HPT_Parameters_Seg_CoteAllard_1.csv')
    ...
    resultsFeatureSet11=evaluation(dataMatrix,segment,classes,peoplePreTrain,peopleTrain,CH,numberFeatures,featureSet,numberShots,combinationSet
                                    ,generalClassifierKNN, pBest, n_neighborsBest, weightsBest,generalClassifierSVM, CBest, kernelBest, gammaBest
    ...
    #dataMatrix=np.hstack((preprocessing.scale(mavMatrix[:,0:CH]),preprocessing.scale(wlMatrix[:,0:CH])
    #                      ,preprocessing.scale(zcMatrix[:,0:CH]),preprocessing.scale(sscMatrix[0,CH])
    #                      ,sscMatrix[CH,:]))
    ...
    resultsParameters=pd.read_csv('HPT_Parameters_Seg_CoteAllard_2.csv')
    n_neighborsBest=resultsParameters['n_neighborsBest'].loc[0]
    weightsBest=resultsParameters['weightsBest'].loc[0]
    pBest=resultsParameters['pBest'].loc[0]
    CBest=resultsParameters['CBest'].loc[0]
    ...
    resultsFeatureSet21=evaluation(dataMatrix,segment,classes,peoplePreTrain,peopleTrain,CH,numberFeatures,featureSet,numberShots,combinationSet
                                    ,generalClassifierKNN, pBest, n_neighborsBest, weightsBest,generalClassifierSVM, CBest, kernelBest, gammaBest
                                    ,nameFile,startPerson,endPerson,allFeatures)
    ...
    resultsParameters=pd.read_csv('HPT_Parameters_Seg_CoteAllard_3.csv')
    n_neighborsBest=resultsParameters['n_neighborsBest'].loc[0]
    weightsBest=resultsParameters['weightsBest'].loc[0]
    pBest=resultsParameters['pBest'].loc[0]
    CBest=resultsParameters['CBest'].loc[0]
    ...
    resultsFeatureSet31=evaluation(dataMatrix,segment,classes,peoplePreTrain,peopleTrain,CH,numberFeatures,featureSet,numberShots,combinationSet
                                    ,generalClassifierKNN, pBest, n_neighborsBest, weightsBest,generalClassifierSVM, CBest, kernelBest, gammaBest
                                    ,nameFile,startPerson,endPerson,allFeatures)
