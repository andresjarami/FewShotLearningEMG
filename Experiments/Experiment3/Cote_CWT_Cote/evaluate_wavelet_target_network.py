...
def calculate_fitness(examples_training, labels_training, examples_test_0, labels_test_0, examples_test_1,
        ...
        for label_index in range(len(labels_test_0)):
            if label_index == dataset_index:
                for example_index in range(len(examples_test_0[label_index])):
                    ...
                    # AJ
                    repetitions.append(len(labels_test_0[label_index][example_index]))
        ...
        for label_index in range(len(labels_test_1)):
            if label_index == dataset_index:
                for example_index in range(len(examples_test_1[label_index])):
                    ...
                    # AJ
                    repetitions.append(len(labels_test_1[label_index][example_index]))
    ...
    return accuracy_test0, accuracy_test1, cl_time
...
if __name__ == '__main__':
    ...
    # Comment between here
    '''
    datasets_pre_training = np.load("data/saved_pre_training_dataset.npy", encoding="bytes", allow_pickle=True)
    ...
    datasets_validation0 = np.load("data/saved_dataset_test0.npy", encoding="bytes", allow_pickle=True)
    examples_validation0, labels_validation0 = datasets_validation0

    datasets_validation1 = np.load("data/saved_dataset_test1.npy", encoding="bytes", allow_pickle=True)
    ...
    for cycles in range(1, 5):
        ...
        for i in range(20):
            accuracy_test_0, accuracy_test_1, cl_time = calculate_fitness(examples_training, labels_training,
            ...
            classificationTime.append(cl_time)
        ...
        save_results = [test_0, test_1, classificationTime]
