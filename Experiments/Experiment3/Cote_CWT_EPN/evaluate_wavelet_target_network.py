...
def calculate_pre_training(examples, labels):
    ...
    cnn = Wavelet_CNN_Target_Network.SourceNetwork(number_of_class=5, dropout_rate=.35).cuda()
...
def pre_train_model(cnn, criterion, optimizer, scheduler, dataloaders, num_epochs=500, precision=1e-8):
    ...
    '''
    .data[0] TO .data
    criterion(outputs, labels) TO criterion(outputs, labels.long())
...
def calculate_fitness(examples_training, labels_training, examples_test_0, labels_test_0, cycles):
    ...
    for dataset_index in range(30):
        ...
        cnn = Wavelet_CNN_Target_Network.TargetNetwork(number_of_class=5,
...
def train_model(cnn, criterion, optimizer, scheduler, dataloaders, num_epochs=500, precision=1e-8):
    ...
    # CHANGES DO NOT SAVE BEST WEIGHTS AJ
