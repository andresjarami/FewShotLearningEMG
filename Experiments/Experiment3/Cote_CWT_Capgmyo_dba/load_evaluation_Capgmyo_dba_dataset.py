...
def read_data(path, modality="cwt"):
    ...
    for subject_index in range(1, 12):
        paths_data = [path + 's' + str(subject_index) + '_0/DB3_s' + str(subject_index) + '/S' + str(
            subject_index) + '_E1_A1.mat']
        train_examples = []
        train_labels = []
        for k, path_data in enumerate(paths_data):
            mat_contents = sio.loadmat(path_data)
            emg_data = mat_contents['emg'][:, ::]  # Get the data from the second EMG
