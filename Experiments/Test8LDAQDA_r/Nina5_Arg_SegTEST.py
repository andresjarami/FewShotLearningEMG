    ...
    for cl in range(1,classes+1):
        ...
        for person in range(1,peoplePreTrain+1):
            if evaluatedPerson!=person:
                ...
                preTrainedDataMatrix.at[indx,'cov']= np.cov(dataMatrix[np.where((dataMatrix[:,allFeatures]==person)*(dataMatrix[:,allFeatures+1]==cl)),0:allFeatures][0],rowvar=False)
                preTrainedDataMatrix.at[indx,'mean'] = np.mean(dataMatrix[np.where((dataMatrix[:,allFeatures]==person)*(dataMatrix[:,allFeatures+1]==cl)),0:allFeatures][0],axis=0)
...
def currentDistributionValues(trainFeatures,trainLabels,classes,allFeatures):
    ...
    for cla in range(0,classes):
        currentValues.at[cla,'cov']=np.cov(Matrix[np.where((Matrix[:,allFeatures]==cla+1)),0:allFeatures][0],rowvar=False)
        currentValues.at[cla,'mean']=np.mean(Matrix[np.where((Matrix[:,allFeatures]==cla+1)),0:allFeatures][0],axis=0)
        ...
        for shot in range(1,numberShots+1):
            for subset in itertools.combinations(combinationSet, shot):
                ...
                for auxIndex in range(np.size(subset)):
                    trainFeatures=np.vstack((trainFeatures,dataMatrix[
                        np.where((dataMatrix[:,allFeatures]==person)                                 
                                 *(dataMatrix[:,allFeatures+2]==subset[auxIndex]))
                        ,0:allFeatures][0]))
                ...
                for auxIndex in range(np.size(diffSubset)):
                    testFeatures=np.vstack((testFeatures,dataMatrix[
                        np.where((dataMatrix[:,allFeatures]==person)                                 
                    ...
                    testLabels=np.hstack((testLabels,dataMatrix[
                        np.where((dataMatrix[:,allFeatures]==person)
                                 *(dataMatrix[:,allFeatures+2]==diffSubset[auxIndex]))
                        ,allFeatures+1][0].T))
                    testRepetitions = np.hstack((testRepetitions, dataMatrix[
                        np.where((dataMatrix[:, allFeatures] == person)
                                 * (dataMatrix[:, allFeatures + 2] == diffSubset[auxIndex]))
                        , allFeatures + 2][0].T))
                ...
                trainedModel11BBLDA, rClass11BBLDA, rClass11BBmeanLDA = reTrainigProposed11BB(currentValues,
                                                                                     preTrainedDataMatrix,
                                                                                     classes, allFeatures,
                                                                                     trainFeatures, trainLabels, step,
                                                                                     'LDA')
...
if featureSet==1:
    ...
    # Getting general data for hyperparameter tunning
    generalDataFeatures=dataMatrix[:,0:allFeatures]
    generalDataLabels=dataMatrix[:,allFeatures+1]
    ...
    resultsParameters=pd.read_csv('HPT_Parameters_Seg_Nina5_1.csv')
    ...
    resultsFeatureSet11=evaluation(dataMatrix,segment,classes,people,CH,numberFeatures,featureSet,numberShots,combinationSet
...
# In[520]:
    ...
    dataMatrix=np.hstack((mavMatrix[:,8:CH*2],wlMatrix[:,8:CH*2],zcMatrix[:,8:CH*2],sscMatrix[:,8:CH*2]))
    #Preprocessing
    dataMatrix = preprocessing.scale(dataMatrix)
    if pca == True:
        pca = PCA(n_components=pcaComp)
        principalComponents = pca.fit_transform(dataMatrix)
        dataMatrix = np.hstack((principalComponents, sscMatrix[:, CH * 2:]))
        allFeatures = np.size(principalComponents, axis=1)
        print('Features: ', allFeatures)
    else:
        dataMatrix = np.hstack((dataMatrix, sscMatrix[:, CH * 2:]))
        allFeatures = numberFeatures * CH
        print('Features: ', allFeatures)
    ...
    resultsParameters=pd.read_csv('HPT_Parameters_Seg_Nina5_2.csv')
    n_neighborsBest=resultsParameters['n_neighborsBest'].loc[0]
    weightsBest=resultsParameters['weightsBest'].loc[0]
    pBest=resultsParameters['pBest'].loc[0]
    CBest=resultsParameters['CBest'].loc[0]
    ...
    resultsFeatureSet21=evaluation(dataMatrix,segment,classes,people,CH,numberFeatures,featureSet,numberShots,combinationSet
                                    ,generalClassifierKNN, pBest, n_neighborsBest, weightsBest,generalClassifierSVM, CBest, kernelBest, gammaBest
                                    ,nameFile,startPerson,endPerson,allFeatures)
    ...
    dataMatrix=np.hstack((lscaleMatrix[:,8:CH*2],mflMatrix[:,8:CH*2],msrMatrix[:,8:CH*2],wampMatrix[:,8:CH*2]))
    ...
    if pca == True:
        pca = PCA(n_components=pcaComp)
        principalComponents = pca.fit_transform(dataMatrix)
        dataMatrix = np.hstack((principalComponents, wampMatrix[:, CH * 2:]))
        allFeatures = np.size(principalComponents, axis=1)
        print('Features: ', allFeatures)
    else:
        dataMatrix = np.hstack((dataMatrix, wampMatrix[:, CH * 2:]))
        allFeatures = numberFeatures * CH
        print('Features: ', allFeatures)
    ...
    resultsFeatureSet31=evaluation(dataMatrix,segment,classes,people,CH,numberFeatures,featureSet,numberShots,combinationSet
                                    ,generalClassifierKNN, pBest, n_neighborsBest, weightsBest,generalClassifierSVM, CBest, kernelBest, gammaBest
                                    ,nameFile,startPerson,endPerson,allFeatures)